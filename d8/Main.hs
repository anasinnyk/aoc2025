...
solve f = do
  ...
  let res = take 1000 $ map fst $ filter (odd . snd) $ zip dists [1 ..]

  print $ product $ map length $ take 3 $ sortBy (\x y -> length y `compare` length x) $ (\arr'' -> nub [sort (i : ns) | (i, ns) <- assocs arr'']) $ fillTheList arr' res
